...
launch_xclip(Pw *pw)
{
	...
	char *cmd;
	// Fail, complete fail. But I dunno how to concatenate strings.
	cmd = launch_find_replace("echo -n \"%p\" | xclip -in -selection \"clipboard\"", "%p", pw->passwd);
	cmd = launch_find_replace("echo -n \"%p\" | xclip -in -selection \"primary\"", "%p", pw->passwd);
	launch_execute(cmd);
}

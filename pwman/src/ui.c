...
ui_run()
{
	...
	while(1){
		...
		if((time_base < (time(NULL) - (options->passphrase_timeout*60)))
			...
			ui_statusline_msg("Password has timed out and you must enter it again.");
			getch();
...
statusline_readonly()
{
	ui_statusline_msg("Password file is opened read-only");
}
...
ui_display_help()
{
	...
	helpwin = newwin(LINES - 6, COLS - 6, 3, 3);
	uilist_clear();
